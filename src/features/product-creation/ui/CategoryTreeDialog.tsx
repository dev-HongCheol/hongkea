  ...
  return (
    <div className="select-none">
      ...
      >
        {hasChildren ? (
          <button
            className="hover:bg-accent/50 flex h-4 w-4 items-center justify-center rounded-sm"
            onClick={(e) => {
              e.stopPropagation();
...
export const CategoryTreeDialog: React.FC<CategoryTreeDialogProps> = ({
  ...
  const handleCancel = () => {
    setTempSelectedCategory(
      selectedCategoryId
        ? categories.find((c) => findCategoryInTree(c, selectedCategoryId)) ||
            null
        : null,
    );
    onOpenChange(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={!tempSelectedCategory}>
            선택
          </Button>
        </DialogFooter>
      </DialogContent>
