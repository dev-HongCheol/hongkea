...
### v1.0.2 (2025-11-30)
- **RLS 정책 개선**:
  - 제품 SELECT 정책: 관리자는 비활성 제품도 조회 가능
  - 제품 UPDATE 정책: `WITH CHECK` 절 추가하여 소프트 삭제 시 정책 위반 해결
  - Supabase Storage RLS 정책 추가: 제품 이미지 업로드/조회/수정/삭제 권한 관리
...
- **상품 관리**:
  - 조회: 일반 사용자는 활성 상품만, 관리자는 모든 상품 조회 가능
  - CUD: 관리자만 생성/수정/삭제 가능
- **주문 정보**: 사용자는 본인 주문만, 관리자는 전체 조회
- **장바구니**: 로그인 사용자는 본인 것만, 비회원은 세션 기반
- **Storage**: 관리자만 업로드/수정/삭제, 모든 사용자는 조회 가능
...
-- 상품 조회: 일반 사용자는 활성 상품만, 관리자는 모든 상품
CREATE POLICY "View products based on role" ON hk_products
    FOR SELECT USING (
        is_active = TRUE  -- 일반 사용자: 활성 상품만
        OR
        EXISTS (          -- 관리자: 모든 상품 (비활성 포함)
...
-- 상품 수정: 관리자만 가능 (WITH CHECK으로 수정 후 검증)
CREATE POLICY "Only admins can update products" ON hk_products
    ...
    WITH CHECK (  -- 수정 후 행도 정책을 만족해야 함 (소프트 삭제 시 필수)
        EXISTS (
            SELECT 1 FROM hk_admin_users a
            WHERE a.user_id = auth.uid() AND a.is_active = TRUE
            AND (
